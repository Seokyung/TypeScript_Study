const shoeSizes: number[] = [220, 230, 240, 250, 260, 270];
...
const clothingSizes: string[] = ["S", "M", "L", "XL"];
...
interface ColorBox<T> extends Box {
	colors: T[];
}

type ColorBoxType = ColorBox<["white", "black", "brown"]>;

// 타입 별칭 제네릭
type Pair<T> = [T, T];
type Triangle<T, U, V> = [T, U, V];
